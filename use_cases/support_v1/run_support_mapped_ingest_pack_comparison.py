...
MAPPED_SAMPLE_A_PATH = SCRIPT_DIR / "helpdesk_export_sample_generic.csv"
...
def merge_mapped_rows(
    ...
    *,
    ...
    required_columns = mapping["columns"]

    for csv_path in csv_paths:
        loaded_rows, fieldnames = load_flat_export(csv_path)
        validate_mapping_against_csv(fieldnames, mapping)
        rows.extend(loaded_rows)
        record_ids.extend(
            str(row[required_columns["record_id"]]).strip()
...
def evaluate_slice(
    ...
    *,
    ...
    rows = merge_mapped_rows(csv_paths, mapping=mapping, slice_name=slice_name)
    normalized_payload = normalize_rows(rows, mapping)
    normalized_payload["dataset_name"] = f"support_v1_mapped_ingest_{slice_name}"
    ...
    return {
        ...
        "input_files": {
            ...
            "mapping_path": format_dataset_path(mapping_path),
        },
        "normalized_dataset": {
...
def build_slice_markdown(slice_result: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        f"- Mapped row count: {slice_result['normalized_dataset']['row_count']}",
        (
...
def build_export_payload(
    ...
    *,
    ...
    return {
        "run_metadata": {
            "evaluation_name": "support_v1_mapped_ingest_pack_comparison",
...
def print_summary_table(
    ...
    *,
    ...
    print("support_v1 mapped-ingest pack comparison")
...
def main() -> None:
    ...
    export_payload = build_export_payload(slice_results, mapping_path=mapping_path)
