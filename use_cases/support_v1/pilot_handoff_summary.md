...
This pack turns the current `support_v1` readiness work into a practical starting point for the first real helpdesk export conversation.

## What `support_v1` currently does

- Ingests a bounded support export slice, normalizes it into the existing `support_v1` case schema, and evaluates routing decisions at labeled decision points.
- Compares default `iml`, calibrated `iml`, `naive_summary`, and `full_history` on the same visible-history slice.
- Uses calibration as the current adjustment layer; core-engine changes are not required for the first pilot.

## What ingest formats already work in prototype form
...
- Existing normalizers/adapters cover the current raw JSON, CSV, mapped CSV, and Zendesk-like prototype paths.
...
- A clear export type choice: raw JSON, Zendesk-like JSON, fixed-contract CSV, or mapped CSV.
...
- The team can explain where calibrated `iml` helps, where guardrails hold, and what data gaps remain before a larger rollout.
