...
from normalize_mapped_support_export import (
    MAPPING_PATH as DEFAULT_MAPPING_PATH,
    OUTPUT_PATH as DEFAULT_NORMALIZED_OUTPUT_PATH,
    SOURCE_CSV_PATH as DEFAULT_CSV_PATH,
    load_flat_export,
    load_mapping,
    normalize_rows,
    validate_mapping_against_csv,
...
DEFAULT_LABELS_PATH = SCRIPT_DIR / "helpdesk_export_sample_generic_labels.json"
MAPPED_INGEST_EVALUATION_PATH = (
    ARTIFACTS_DIR / "latest_support_mapped_ingest_label_evaluation.json"
)
MAPPED_INGEST_REVIEW_PATH = (
    ARTIFACTS_DIR / "latest_support_mapped_ingest_label_review.csv"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Normalize a mapped external support CSV into the support_v1 case schema "
    ...
    parser.add_argument(
        "--csv-path",
        type=Path,
        help="Override the external support CSV export path.",
    )
    parser.add_argument(
        "--mapping-path",
        type=Path,
        help="Override the column-mapping JSON path.",
    )
    parser.add_argument(
        "--labels-path",
        type=Path,
        help=(
            "Override the support label dataset path. Defaults to the mapped-sample "
            "label set for the generic helpdesk export."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    mapping_path = resolve_input_path(args.mapping_path, DEFAULT_MAPPING_PATH)
    ...
    rows, fieldnames = load_flat_export(csv_path)
    ...
    export_payload["run_metadata"]["evaluation_name"] = (
        "support_v1_mapped_ingest_label_evaluation"
    )
    export_payload["run_metadata"]["source_csv_path"] = format_dataset_path(csv_path)
    export_payload["run_metadata"]["mapping_path"] = format_dataset_path(mapping_path)
