...
This pack makes the first Support V1 pilot deliverable concrete. It defines what the client provides, what the Support V1 team does during the pilot, what the client gets back at the end, what is not included, and what next step is recommended.

## What the client provides

- One bounded support export slice in a supported format, preferably the raw JSON export path.
...
## What the Support V1 team does during the pilot

- Validates whether the supplied export fits the current contract and flags blockers early.
- Normalizes the pilot slice into the current Support V1 workflow without core-engine changes.
- Runs the pilot evaluation on the real slice and compares standard routing with calibrated routing.
- Reviews auditability, redaction safety, joins, and visible-history reconstruction on the scoped slice.
- Prepares decision-ready handoff materials for the client review owner.
...
- Normalized export and evaluation result: one auditable pilot slice in the current Support V1 schema plus the corresponding evaluation outputs for the scoped review set.
- Calibrated routing comparison: a reviewable comparison showing standard routing, calibrated routing, and the current comparison methods on the same pilot slice.
...
- Pilot deliverables and handoff materials: the summary materials needed for an internal buyer, operator owner, or sponsor to review the pilot result without re-running the analysis.
...
- Run a second pilot on another supported export format if the client's natural export requires it.
