...
This one-pager turns the current `support_v1` readiness work into a short commercial pilot artifact for someone deciding whether to approve a first trial.

## What problem `support_v1` solves

Support teams often have to decide which cases can stay on a fast path and which need deeper review, but that decision is slowed down by messy export data, incomplete case-history reconstruction, and inconsistent manual triage. `support_v1` is designed to take a bounded helpdesk export, reconstruct visible ticket history, and support a more reliable routing decision without requiring a core-engine rewrite.

## What is already built

- Prototype ingest paths for raw JSON, flat CSV, mapping-based CSV, and Zendesk-like nested exports.
...
- Normalization and evaluation runners that compare `iml`, calibrated `iml`, `naive_summary`, and `full_history` on the same labeled decision points.
- Pilot packaging, handoff, onboarding, and workspace materials for a first bounded external pilot.

## What evidence exists so far

- Current evidence is directionally strong across the implemented `support_v1` modalities.
- On the largest slice in each loaded modality, calibrated `iml` beats the best non-calibrated method in 5 of 5 modalities.
...
- Pilot packages, handoff bundles, and one validated real pilot workspace already exist.

## What the first pilot would look like

- One customer or partner.
- One queue or workflow in scope.
- One export modality, preferably `raw_ingest` unless another format is the customer's natural export.
...
- A reviewable comparison artifact showing calibrated `iml` against the current baselines on the pilot slice.
...
- `raw_ingest` is the strongest starting path; other modalities are usable, but some have thinner evidence.
